...
- 文件操作应遵从 Agent 的工具调用。
- 原则上不允许使用 Python、Bash 等直接读写文件，除非遇到需要输出经过复杂计算的内容。
...
- 不要混用单引号和双引号：单引号中的内容通常按字面值处理，双引号中的变量和子表达式会被展开。应根据是否需要插值选择引号。
- 使用反斜杠 `\` 时，先确认它在当前命令、字符串或正则表达式中是普通字符还是转义符，并根据实际的解析层级决定是否需要重复转义。
